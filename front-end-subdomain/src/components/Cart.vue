...
export default {
 ...
 data: () => ({
   headers: [
      { text: '이름', value: 'clothe.name' }, // text: table의 column 이름 value: backend의 pet name
      { text: '최종 결제 옷', value: 'clothe.addbutton' }, // text: table의 column 이름 value: backend의 pet energy
      { text: '좋아요', value: 'clothe.likebutton' }, // text: table의 column 이름 value: backend의 pet appearance
    ...
    methods:{
      ...
      async init(){
        ...
        this.cartItems.forEach(async cartItem => {
          const response = await axios.get(new URL(cartItem._links.clothe.href).pathname); 
          //backend에서 cartItem에 있는 clothes의 pathname 불러오기
          cartItem.clothe = response.data;
        })
      },
    }
